...
async function initializeMongoDB() {
  ...
  const existingGurusIds = await Guru.find({
    guildId: { $in: gurusIds.flat() },
  });
...
client.on('ready', async (client) => {
  ...
  if (channel instanceof TextChannel) {
    ...
    await channel
      ...
      .then(() =>
        console.log(
          `Mensagem enviada com sucesso para o canal ${channel.name}!`
        )
      )
      .catch((error: any) =>
        console.error(
          `Erro ao enviar mensagem para o canal ${channel.name}:`,
          error
        )
      );
  } else {
