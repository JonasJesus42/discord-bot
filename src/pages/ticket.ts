...
import {createId, getUserName, readId} from '../utils';
import {getGurusNamesIds} from "../client/googleSheets";
...
export function selectComponent(): InteractionReplyOptions {
  const embed = new EmbedBuilder().setTitle(
    'Selecione a Áreas de Conhecimento'
  );
...
export function buttonSendTicketComponent(
  ...
  const embed = new EmbedBuilder()
    .setTitle(
      `Antes de enviar o ticket para equipe de ${categoryName} certifique de:`
    )
    .setDescription(
      '1° Ter passado 30 minutos para debugar ou tentado achar uma solução\n ' +
  ...
  const component = [
    new ActionRowBuilder<ButtonBuilder>().addComponents(sendTicket),
  ];
...
export function embedSupportRequest(
  customId: string,
  guildId?: string,
  disabled: boolean = false
): InteractionReplyOptions {
  const [ userName, userId, stack] = customId.split(';');
  const embed = new EmbedBuilder()
    .setTitle(`${userName} Solicita um soporte técnico para ${stack}`)
    .setDescription('Voce tem a disponibilidade para atender ?');

  const acceptedId = createId(
    Namespaces.acceptedTicket,
    userName,
    userId,
    guildId,
  );
  ...
  const component = [
    new ActionRowBuilder<ButtonBuilder>().addComponents(refuse, accepted),
  ];
